...
// Tyler Blanchard's profile data directly from Pillar screenshots
const tylerProfile = {
...
export function AddTylerButton() {
  const [isLoading, setIsLoading] = useState(false);
  const { toast } = useToast();
  const { refetch: refetchCreators } = useQuery({ queryKey: ['/api/creators'] });

  const addTylerProfile = async () => {
    setIsLoading(true);
    try {
      ...
      toast({
        title: "Tyler Blanchard profile added",
        description: "The creator profile with complete Pillar data has been added.",
        variant: "default",
      });
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Failed to add Tyler profile",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
