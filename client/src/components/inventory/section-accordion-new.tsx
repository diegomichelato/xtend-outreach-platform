      ...
      {sectionNames.map((sectionName) => (
        <AccordionItem key={sectionName} value={sectionName}>
          ...
          <AccordionContent className="pt-4 pb-2 px-2">
            ...
            <Accordion type="multiple" className="w-full">
              {renderCreatorGroups(sectionName, groupedBySection, selectedProjects, setSelectedProjects, setSelectAll, filteredData, getStatusColor)}
            </Accordion>
          </AccordionContent>
        </AccordionItem>
      ))}
...
function renderCreatorGroups(
  ...
  // Process all creators in this section
  ...
  // Create array of AccordionItems for each creator group
    ...
    <AccordionItem key={`${sectionName}-${creatorName}`} value={`${sectionName}-${creatorName}`}>
      <AccordionTrigger className="px-4 py-2 bg-slate-50 rounded-t-md hover:bg-slate-100">
        <div className="flex items-center justify-between w-full">
          ...
          <Badge>{creatorsInGroup.length} videos</Badge>
        </div>
      </AccordionTrigger>
      <AccordionContent>
                ...
                <TableHead className="w-10">
                  <div className="flex items-center">
                    <input
                      ...
                      checked={creatorsInGroup.every(creator => selectedProjects.includes(creator.id))}
                      onChange={(e) => {
                        if (e.target.checked) {
                          const newSelectedProjects = [...selectedProjects];
                          creatorsInGroup.forEach(creator => {
                          ...
                          setSelectedProjects(selectedProjects.filter(id => 
                            !creatorsInGroup.some(creator => creator.id === id)
                          ));
                        }
                      }}
                    />
